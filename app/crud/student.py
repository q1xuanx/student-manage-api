...
async def add_student(conn: Connection, student: StudentModel) -> bool:
    status = await conn.execute(
        "INSERT INTO students(namestudent, dob, faculty, classid) values($1, $2, $3, $4)",
...
async def update_student(conn: Connection, student: UpdateStudentModel) -> bool:
    status = await conn.execute(
        "UPDATE students SET namestudent=$1, dob=$2, faculty=$3, classid=$4 WHERE idstudent=$5",
        student.name_student,
        student.dob,
        student.faculty,
        student.class_id,
...
async def check_exist_student(conn: Connection, student: StudentModel) -> bool:
    status = await conn.fetchrow(
        "SELECT idstudent FROM students WHERE namestudent ILIKE $1 AND dob = $2 AND faculty = $3",
        student.name_student,
        student.dob,
        student.faculty,
    )
...
async def search_student(
    ...
    get_total_query = base_query.replace(
        "SELECT idstudent, namestudent, faculty, s.classid, dob ", "SELECT COUNT(*) "
    )
    ...
    return {"status": True, "list": list_student, "total": total_student}
